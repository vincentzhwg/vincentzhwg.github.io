...
<strong>Triggering Mang Tasks</strong>
...
<strong>发送error告警邮件的配置示例</strong>
...
<strong>以daemon方式启动luigid</strong>
...
class BF(luigi.Task):
    ...
    def run(self):
        ...
        hdfsClient.put( self.input().path, self.output().path )</pre>
[/python]

<strong>执行hive语句</strong>
...
class P(luigi.hive.HiveQueryTask):
    ...
    def query(self):
        ...
        return """
            ...
            """.format(ym=date.strftime("%Y%m"), d=date.strftime("%d"), appid=self.appid)</pre>
[/python]
...
<strong>可加jar包的HivePartitionTarget</strong>
[python]
#!/usr/bin/env python
#-*- coding:utf8 -*-
